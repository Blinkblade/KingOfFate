...
`docs/phase_reports/P2-base-fighter-template.md` 的 Runtime Validation 一节）。
...
| 3000+ | 超杀 | **未实现，预留** |
| 5000–5210 | 受击 / 倒地 / 起身 | **公共状态**（勿自实现） |

模板自实现的状态：195、200、210、230、240、800、810、1000、1010。
`hits.zss` 是受击方专用状态（投技受害者流程）的落点，目前只含 810 配套。
...
- **气槽** = IKEMEN 原生 `power`。`_template.const` 的 `power = 1000` 是
  **上限**（不是起始值！每回合开始引擎清零，KFM 同样如此 —— 本机实测，
  `char.go:3740` `c.powerMax = gi.data.power`）。
- **收入**：`hitDef` 命中给气（引擎按 `default.attack.lifetopowermul` 折算，
  实测轻拳命中 POW 0→37、重拳 0→84）+ 状态头 `poweradd`（必杀进入 +40）。
- **支出**：EX 在路由层检查 `power >= 500`，State 1010 内 `powerAdd{value: -500}`
  扣除（实测 1000 → 500，帧证据 `v13_ex_p06_0D_burst07.png`）。
- **取消**：`command.zss` 的 `Combo()` 函数返回"现在能否出必杀"：
  地面自由态，或 State 200/230 的 `animElemTime(3) >= 0`（首个攻击帧之后）
  且 `var(0) = 0`。取消发生时状态内把 `var(0)` 置 1 闩锁。
  实测：A 起手 → 取消窗口内 QCF+A → State 1000 接管（`v14_cancel_*`）。
...
- 引擎热键（`F3` 充满气等）**可以**被合成注入触发（P2 实测，V13 靠它攒气）；
...
1. 蹲攻（400–440）、跳攻（600–640）、超杀（3000+）、受身（recovery 命令已定义，
   状态未实现）：状态号与命令已预留，实现即扩展。
2. B/D 键与嘲讽的运行时证据依赖真人键盘（本机合成注入不可达）。
3. 精灵/音效是 KFM 占位 —— 每个克隆角色必须替换（§9）。
4. `_template.air` 的攻击帧时长（State 200 共 20 tick）是"可观测性优先"的
   调参值，正式角色按手感调。
5. 取消窗口刻意不要求 `moveContact`（空挥可取消）—— 降低联调门槛；
