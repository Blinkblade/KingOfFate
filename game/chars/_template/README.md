...
> **★★★ P4 补充：攻击框有"逐帧"与"默认"两种声明，用途不同 ★★★**
>
> | 写法 | 作用范围 | 什么时候用 |
> | --- | --- | --- |
> | `Clsn1: N` + `Clsn1[i] = …` | **只作用于紧邻的那一帧** | 普通招式：判定帧有限，判定之后显式写 `Clsn1: 0`（本模板的 Action 200/210/600… 都是这样） |
> | `Clsn1Default: N` + `Clsn1[i] = …` | **整个动画** | **投射物、以及任何"全程都该有判定"的动画** |
>
> **这是 P4 花了最多时间的一个坑**：投射物动画的末帧是 `-1`（无限保持），
> 用 `Clsn1:` 声明时攻击框只覆盖第一帧（几 tick），之后投射物就变成
> **"飞得很好看、但打不中人"的哑弹** —— 而且没有任何报错。
> 症状很好认：**小判定框全不命中，只有超大判定框才命中**
> （超大框的水平范围大到在生成的瞬间就够到对手）。
> 改 `Clsn1Default` 后立刻正常。
>
> **判断法则：动画里如果有 `-1` 保持帧并且需要判定，就必须用 `Clsn1Default`。**
...
> **★ P4 修正（实测）**：走路**只有一个状态号 `20`**，**没有状态 21**。
> `common1.cns.zss:73-88` 的 `[StateDef 20]` 按 `command = "holdback"` /
> `"holdfwd"` 设置速度，再按速度方向选**动画** 20（前）或 21（后）——
> 也就是说 **21 是动画号，不是状态号**。
> 写 `changeState{value: 21}` 会让引擎报
> `in state 21: changed to invalid state 21`。
> 本表此前写成 "20–21" 是错的，已修正。任何"让角色走路"的代码都必须用 `20`。
