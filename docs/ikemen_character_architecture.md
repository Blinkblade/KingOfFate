...
| 常量 | E1 | 改 `[Velocity] walk.fwd` → 移动速度变化 |
...
| 编号段 | 约定用途 | 例子 |
| --- | --- | --- |
| `0` / `5` / `10`–`20` | 站立待机 / 走路 / 转向 | `StateDef 0` 在 `data/common1.cns.zss` |
| `40`–`60` | 跳跃（起跳/上升/下落/落地） | `changeState{value: 40}` → 跳跃 |
| `100`–`105` | 前跑 / 后跑 | `kfm.zss` 的 `StateDef 100` / `105` |
| `120` | 站立防御（引擎 `autoguard`） | `AI.zss:174` `changeState{value: 120}` |
| `130`–`155` | 通用攻防受击段 | `data/common1.cns.zss` |
| `195` | 嘲讽（KFM 约定） | `kfm.zss:196` `[StateDef 195]` |
...
| `800`–`899` | **投技** | `800` 投技起手 |
| `1000`–`1499` | **必杀技（special）** | `1000`/`1010` 金刚掌 x/y；`1400`–`1420` Zankou |
| `3000`–`3999` | **超必杀（super）** | `3000` TripleKFPalm、`3050` SmashKFUpper |
| `5000`–`5299` | **通用受击/倒地/起身**（引擎公共） | `5040` 受击浮空、`5120` 倒地起身、`5300` 眩晕 |
...
**`[StateDef -1]` 是特殊状态**：它每 tick 都执行一次，与当前状态号无关。
`command.zss` 与 `AI.zss` 都用了它，这也是"命令路由"和"AI 决策"能够每 tick 生效的原因
（出处：`command.zss:71` `[StateDef -1]`；`AI.zss:23` `[StateDef -1]`）。

**`[StateDef -4]` 是 AI 的全局状态**：不受 pause / superPause 停止，也没有 helper 限制
（出处：`AI.zss:1-4` 的注释与 `[StateDef -4]`）。
...
| `options.lua` | 选项界面 |
| `default.lua` | 默认行为 |
| `debug.lua` | **调试覆盖层**：`Ctrl+C` 判定框、`Ctrl+D` 状态读数 |

**P1 直接用到的两个钩子**（`debug.lua`）：

| 快捷键 | 功能 | 用途 |
| --- | --- | --- |
| `Ctrl+C` | `toggleClsnDisplay` | 显示红/蓝判定框（E4 证据） |
| `Ctrl+D` | `toggleDebugDisplay` | 左下角打印 `State No / CTRL / Type / MoveType / Physics / Time / LIF / POW`（E2/E3/E5/E6 证据） |

> **注意**：`F1`–`F10`、`SPACE`、`PAUSE`、`SCROLLLOCK` 已被引擎自身占用，不能用作探测按键。
> 这是 P1 在搭建截图工具时踩到的坑，详见 `docs/p1_experiments.md` §2。

**结论**：Lua 层适合做**观测、调试、界面**，不适合做角色战斗逻辑——
...
| 需求 | 应该在哪儿做 | 是否碰到引擎 | 证据 |
| --- | --- | --- | --- |
| 改移动速度、跳跃高度、体力、被击退 | 角色 `.const` 的 `[Velocity]` / `[Data]` / `[Size]` | **否** | E1：`walk.fwd 2.4 → 12.0` 生效（`kfm.const:55`） |
