...
Gate 6 卡住的原因一直被写成"要有人按键"。这一轮把装置补齐了，但**取消序列本身仍未取到**：

| 做了什么 | 结果 |
| --- | --- |
| 给 `framestep_probe.ps1` 补上键位快照/还原（`inject_phases` 早已有，步进脚本没有） | ✅ 已补，运行前后 `config.ini` 哈希一致 |
| 修 `framestep_probe.ps1` 的 `AttachThreadInput` 崩溃 | ✅ 根因：`GetWindowThreadProcessId` 被声明成返回 `IntPtr`（Win32 实际返回 DWORD），
  传给 `AttachThreadInput(uint,uint,bool)` 时**间歇性**报"无法将值 10576 的 IntPtr 转换为 UInt32"。
  改成 `uint` 后不再复现（`inject_phases` 的声明本来就是 `uint`，所以只有步进脚本中招） |
| 逐 tick 跑通一次（`-Steps '0x09:2,none:3,0x28:1,0x28+0x27:1,0x27:1,0x09:1,none:26'`） | ⚠️ 拿到：按 x 的那一 tick 直接进入 **`State No: 1000`**，其后 26 tick 持续 1000。
  但**前置的 200 没有出现**（开头的 x 落在回合开始不可控期），所以这不构成"从 200 取消"的证据 |
| 加前置空推再测（`-Steps 'none:6,0x09:4,...'`） | ❌ 该轮**所有 `Save-Shot` 返回 FAILED**，一帧都没存下来；换用 `inject_phases` 连拍的一次则以 `EXIT=1` 提前结束 |

**当前结论**：Gate 6 仍 **BLOCKED**。已确认的是"236+A 能进状态 1000"；未确认的是
"从 200 的取消窗口内切进去"。下一步按顺序排查：`Save-Shot` 在暂停态失败的原因
（`PrintWindow` 对 OpenGL 窗口本就会阻塞渲染线程），或改用"暂停 → 单步 → 只在关键 tick 截图"
的轻量步进模式，避免每 tick 都拍。在此之前不写 PASS。
...
| 被跳跃规避 | 脚本假人 `assertInput{flag: U}` + 加大跳跃初速 | ✓ **PASS**：`1000 → 1000`（**零伤害**，而 P1 已发波 `POW 40`） |
...
| `guard`（`autoGuard`） | 1000 → **994**（-6） | **PASS**：防御成立，只吃 6 点 |
| `jump`（第一版：仅按住上） | 1000 → **940**（-60） | **与对照组无差别 ⇒ 证明不了任何事**（见下） |
| `jump`（第二版：加大跳跃初速） | 1000 → **1000**（0） | **PASS**：波从空中假人身下穿过 |

证据：`docs/evidence/p4/gate7_dummy_matrix.txt`（含每个帧的读数）。

**第一版 `jump` 为什么没做成（值得记下来）**：按住上会让假人**反复起跳落地**，而
模板的跳跃参数 `jump.neu = 0,-8.4`、`yaccel = .44` 只给约 38 tick（≈0.63 s）滞空 ——
短于波飞越全场所需时间，所以命中瞬间它已经落回地面，读数与站着不动**完全相同**。
修法是让假人的滞空覆盖飞行时间：`make_dummy.ps1` 给跳跃假人把 `jump.neu` 改成 `0,-25`
（≈114 tick ≈ 1.9 s）。这是**改假人，不是改被测角色** —— 被测的 Test Fighter B 一行没动。
