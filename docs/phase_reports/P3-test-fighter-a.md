...
## 8. 复查（2026-09-17，交付后审计）

按 P2 审计的同一方法（结论回到代码与原始帧）把 P3 的代码与结论全部重对了一遍，
发现并修复 **4 类问题**；修复后复跑：`test.ps1` 26/26、加载 0 崩溃日志、
蹲重拳仍命中（P2 LIF 940 = −60）。

| # | 问题 | 性质 | 处置 |
| --- | --- | --- | --- |
| 1 | **攻击框未在判定帧后清零**：`.air` 里 `Clsn1` 会沿用到下一次声明（MUGEN/IKEMEN 语义；旁证：模板 Action 200/240 都在收招帧显式写 `Clsn1: 0`，若框不沿用则这两行无意义 —— **尚未 A/B 实测**），新动作（400/410/430/440/600/610/630/640/1100）都没写 `Clsn1: 0`，实际判定窗口 = 判定帧→收招结束，比 `moves.csv` 的 Active 长（最长多 24 tick） | **功能差异**（连段/对空判定比文档更"黏"） | 全部补 `Clsn1: 0`（1000/1010/3000 与站立四键原本就有）；在 Action 400 的注释里写明这条语义，防止将来被"清理" |
| 2 | **Super 突进距离写错**：文档与注释写 44 px，代码实际 −6+18+16+10 = **38 px** | 文档错误 | `.zss` 注释、角色 README、迭代记录改为 38 px |
| 3 | **`moves.csv` 的 Cancel Into 两处错误**：200/230 漏了 240（同为 lv2 目标）；400 错误列出 430（lv1 不能取消进 lv1，且站姿攻击期间 `stateType = S` 也进不了蹲姿路由） | 数据表错误 | 已修 |
| 4 | 注释遗留：`.air` 里 1000/1010 仍叫 "Placeholder"；1100 注释的元素号自相矛盾（elem 6 vs 7）；610/640 的收招帧数笔误（15→12、48→8）；`movelist.dat` 漏了后投（代码支持 `holdback` 后投） | 文档/注释错误 | 全部修正；630 动作首帧沿用 KFM 的 `640,0` 精灵这一继承行为加了注释说明 |

复查中的一次**操作事故**（记录备查）：编辑 `.air` 时替换串选错目标，把 Action 630
的 `630,2, 0,0, 12` 改成了 `430,3, 0,0, 4`、受击框也被换成了蹲姿框 ——
靠"编辑后重读目标块"发现并完整还原。教训：**对同一文件做多处相似结构的编辑时，
每一处编辑后都要回读确认**。
