...
| # | 问题 | 性质 | 处置 |
| --- | --- | --- | --- |
| 1 | **攻击框未在判定帧后清零**：`.air` 里 `Clsn1` 会沿用到下一次声明，新动作（400/410/430/440/600/610/630/640/1100）都没写 `Clsn1: 0`，实际判定窗口 = 判定帧→收招结束，比 `moves.csv` 的 Active 长（最长多 24 tick） | **功能差异**（连段/对空判定比文档更"黏"） | 全部补 `Clsn1: 0`（1000/1010/3000 与站立四键原本就有）；在 Action 400 的注释里写明这条语义，防止将来被"清理" |
