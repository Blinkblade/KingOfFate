...
收尾后对全部结论做回溯审计（代码 → 原始帧 → 必要时实测），修正 13 处
...
- **计数类**：模板是 **12 个文件**（文档原写 11）；带 `_template` 前缀的文件是
  **7 个**（原写 8），另外 5 个（`command.zss` / `hits.zss` / `AI.zss` /
  `movelist.dat` / `README.md`）不改名 —— 手册里补上了这份"不改名清单"。
