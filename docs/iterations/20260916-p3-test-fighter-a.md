...
| Super | 3000 | 236236+A：伤害 200、突进 38 px、击倒且不可受身，耗 1000 气 |
...
> **2026-09-17 复查补充**：交付后按 P2 审计方法重对代码与结论，发现并修复 4 类问题
> （攻击框未清零导致判定窗口比文档长、Super 突进距离 44→38 px、moves.csv 的
> Cancel Into 两处错误、若干注释/出招表遗留）。修复后复跑：`test.ps1` 26/26、
> 加载 0 崩溃日志、蹲重拳仍命中 60。明细见 Phase Report §8。
