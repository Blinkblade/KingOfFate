...
| 运行 | 投射物命中 | 对手 `LIFE 1000 → 940`（伤害 60 精确） |
| 运行 | 投射物 K.O. | `PERFECT!!` / `YOU LOSE!` |
| 运行 | **Super** | `State 3000`，`POW 2000 → 1000`（正好 1000） |
| 运行 | **EX** | `State 1010`，画面**同时 2 枚投射物**，`POW 580 → 80`（正好 500） |
| 运行 | **Anti-Air** | `State 1100` 触发（`ElemNo 8/14`）；"空中命中"未取到截图 |
| 运行 | **A vs B AI 对战** | **110 秒**，`crashlogs: 0 new`；B 把 A 打到 74 血 |
| 运行 | 长手 210 | `State 210`，`ElemNo 5/8`（正是设计的判定帧） |
| 运行 | 取消链 | 状态序列 `200 → 1000`（时间序不精确，见 §6） |
