...
- `docs/p1_experiments.md` + `docs/evidence/p1/` —— 6 个独立实验的完整记录与原始证据
  （11 张图 + 12 份运行报告 + 1 份像素测量，约 3.73 MB）
- `design/characters/_template/` —— P2 可直接复制的角色骨架（10 个文件 + 使用说明）
- `tests/p1/` —— 可复用的角色行为观测工具（无人值守开局 / 输入注入 / 抓帧 /
  状态读数拼接 / **按像素测量角色位置**）
...
| # | 改什么 | 观测到什么 |
| --- | --- | --- |
| E1 | `.const` `walk.fwd 2.4 → 12.0` | 同时间位移 **21 px → 108 px，比值 5.14 ≈ 5.0** |
...
收尾自查时逐条核对结论与证据链，发现两个真问题，都已修正并保留了完整记录：

1. **E1 的第一版证据无效。** 它把调试覆盖层上 `P1: 56` 里的数字当成了位置读数，
   而 `debug.lua:183-184` 显示那个字段是 `id()`（角色 ID），与坐标无关，
   不同运行之间本来就会变。已移除 4 张作废证据，改为按像素测量角色名标签的中心，
   重做后得到比值 **5.14**（对应 5.0）。
2. **注入的按键会被 AI 静默接管。** 观测工具的 AI 等级默认是 8，AI 直接 `changeState`
   并且对默认走路下了 `assertSpecial{flag: nowalk}`，注入的方向键完全无效 ——
   而画面里角色确实在动（AI 自己在动），不容易察觉。已作为硬前提写入所有相关文档，
   并给出自查方法（运行报告的 `args` 行若出现 `-p1.ai` 即说明该轮注入无效）。

这一轮还补充了：`[StateDef -2]` / `-3`（原先只写了 `-1` / `-4`）、
`debug.lua:32-48` 的 9 个未使用调试热键（人工调试很有用，但未验证可注入）、
以及 kickoff 要求的文档合并取舍说明。
...
- 运行时键位（`save/config.ini` 的 `[Keys_P1]`）→ 已还原为原始值
